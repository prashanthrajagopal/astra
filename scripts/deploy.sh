...
if [[ "$DEPLOY_MODE" == "gcp" ]]; then
  ...
  # --- Setup (first-time infra provisioning) ---
  if [[ "$GCP_SETUP" == "true" ]]; then
    ...
    gsutil mb -l "$GCP_REGION" "gs://${GCP_PROJECT}-astra-workspace" 2>/dev/null || echo "  (already exists)"
  ...
  # Extract Cloud SQL IP from values file or gcloud
