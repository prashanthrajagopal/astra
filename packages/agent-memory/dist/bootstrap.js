...
async function main() {
    ...
    for (const mem of SEED_MEMORIES) {
        ...
        const entry = {
            ...
            expiresAt: (0, redis_1.computeExpiresAt)(mem.type, now),
        };
        try {
            const embedding = await (0, embeddings_1.embed)(entry.summary);
            await (0, redis_1.storeMemory)(entry, embedding);
            stored++;
            process.stderr.write(`  [${mem.type}] ${entry.summary.slice(0, 80)}...\n`);
        }
        catch (err) {
            process.stderr.write(`  WARN: Failed to store memory: ${err instanceof Error ? err.message : String(err)}\n`);
        }
    }
