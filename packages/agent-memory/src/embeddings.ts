      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            const parsed: OllamaEmbedResponse = JSON.parse(data);
...
export async function healthCheck(): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    const req = http.request(
      { hostname: url.hostname, port: url.port, path: url.pathname, method: 'GET', timeout: 5_000 },
      (res) => {
        let data = '';
        res.on('data', (chunk) => (data += chunk));
        res.on('end', () => {
          try {
            ...
            const hasModel = parsed.models?.some((m: { name: string }) =>
