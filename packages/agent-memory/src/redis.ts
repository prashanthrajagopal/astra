...
export function getRedis(): Redis {
  if (!client) {
    client = new Redis(REDIS_URL, { lazyConnect: true, maxRetriesPerRequest: 3 });
  }
  return client;
}
...
const TTL_DAYS: Record<MemoryEntry['type'], number> = {
  decision: 365,
  pattern: 365,
  work_summary: 180,
  investigation: 90,
...
export function computeExpiresAt(type: MemoryEntry['type'], createdAt: Date): string {
  const expires = new Date(createdAt);
  expires.setDate(expires.getDate() + TTL_DAYS[type]);
  return expires.toISOString();
}

export async function storeMemory(entry: MemoryEntry, embedding: number[]): Promise<void> {
  const redis = getRedis();
  ...
  if (entry.detail) metaPayload.detail = entry.detail;
  if (entry.files?.length) metaPayload.files = entry.files.join(',');
  if (entry.layer) metaPayload.layer = entry.layer;
  if (entry.sourceTranscriptId) metaPayload.sourceTranscriptId = entry.sourceTranscriptId;
  ...
  await redis.call(
    'VADD', VECTOR_KEY,
    'VALUES', String(VECTOR_DIM), ...floatArgs,
    entry.id
  );
}
...
export async function queryMemory(
  embedding: number[],
  topK: number = 5,
  filters?: { type?: string; project?: string; layer?: string }
): Promise<MemoryEntry[]> {
  const redis = getRedis();
  ...
  for (const id of rawResults) {
    ...
    if (filters?.layer && meta.layer !== filters.layer) continue;
